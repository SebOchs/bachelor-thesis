...
# preprocessor for beetle and scientsbank for albert
...
class AlbertPreprocessor:
    ...
    def __init__(self, tokenizer, data=None, data_path=None, save_path=None, mode='test', split=0.95, cls=['[CLS]'],
                 ...
                 max_len=128):
        """
        Initialize an instance of AlbertPreprocessor
        ...
        if None != self.data and None == self.data_path:
            ...
            for file in files:
                ...
                for ref_answer in root[1]:
                    # If there are multiple reference answers (beetle), only choose those who are marked as best
                    if ref_answer.get('category') is not None:
                        if ref_answer.get('category') != 'BEST':
                            continue
...
if __name__ == '__main__':
    # Tokenizer
    pretrained_weights = 'albert-large-v1'
    tokenizer = AlbertTokenizer.from_pretrained(pretrained_weights, do_lower_case=True, do_basic_tokenize=True)
    # Beetle
    AlbertPreprocessor(tokenizer, data_path='../data/datasets/training/beetle', save_path='../data/preprocessed/albert_beetle',
                     mode='train').preprocess()
    AlbertPreprocessor(tokenizer, data_path='../data/datasets/testing/beetle/test-unseen-answers', save_path='../data/preprocessed/albert_beetle_ua',
                     mode='test').preprocess()
    AlbertPreprocessor(tokenizer, data_path='../data/datasets/testing/beetle/test-unseen-questions',
                     save_path='../data/preprocessed/albert_beetle_uq',
                     mode='test').preprocess()
    # scientsBank
    AlbertPreprocessor(tokenizer, data_path='../data/datasets/training/sciEntsBank', save_path='../data/preprocessed/albert_sciEntsBank',
                     mode='train').preprocess()
    AlbertPreprocessor(tokenizer, data_path='../data/datasets/testing/sciEntsBank/test-unseen-answers',
                     save_path='../data/preprocessed/albert_sciEntsBank_ua',
                     mode='test').preprocess()
    AlbertPreprocessor(tokenizer, data_path='../data/datasets/testing/sciEntsBank/test-unseen-questions',
                     save_path='../data/preprocessed/albert_sciEntsBank_uq',
                     mode='test').preprocess()
    AlbertPreprocessor(tokenizer, data_path='../data/datasets/testing/sciEntsBank/test-unseen-domains',
                     save_path='../data/preprocessed/albert_sciEntsBank_ud',
                     mode='test').preprocess()
